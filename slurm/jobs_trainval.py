...
def make_slurm_train(job_name,
               ...
               heads=4,
               launch=False):
    job_name = f"slurm/{city_train}_train_val_2.slurm"
    start_script = ("#!/bin/bash\n" +
                    f"#SBATCH --job-name=regression\n" +
                    "#SBATCH --output=output_ruche/%x.o%j\n" +
                    "#SBATCH --time=24:00:00\n" +
    ...
    pattern = f"{city_train}_{city_test}"
    command_create = f"python src/create_data.py --cities_test {city_test} --cities_train {city_train} --name {pattern}\n"
    exp_name = f"{city_train}_train_{city_test}_test"
    ...
    command = "python src/main.py " +\
              ...
              f"--data_dir {data_dir} " +\
              "--data_class hur " +\
              f"--pattern {pattern}SINGLETRAIN " +\
              f"--val_pattern {pattern}SINGLEVAL " +\
              f"--epochs {epochs} " +\
              "--optimizer RAdam " +\
              f"--batch_size {batch_size} " +\
              f"--d_model {d_model} " +\
